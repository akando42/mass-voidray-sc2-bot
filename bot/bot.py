...
class EnemyBot(BotAI):
    NAME: str = "SplitAttackTerran"
    ...
    async def on_start(self):
        print("EnemyBot started")

    async def on_step(self, iteration):
        # Uses:
        # --EnemyOffense "22,81|51,74|88,69"

        coords = self.args.EnemyOffense.split("|")

        if len(coords) < 3:
            return

        p1 = Point2(tuple(map(int, coords[0].split(","))))
        p2 = Point2(tuple(map(int, coords[1].split(","))))
        p3 = Point2(tuple(map(int, coords[2].split(","))))

        # No mining
        # No production
        # No building
        # Only split current army

        await self.split_attack_3(p1, p2, p3)

    async def split_attack_3(self, t1, t2, t3):
        ...
        units = army.sorted(lambda u: u.tag)

        total = len(units)
        third = max(1, total // 3)

        group1 = units[:third]
        group2 = units[third:third * 2]
        group3 = units[third * 2:]
    ...
    async def on_end(self, result):
        print("EnemyBot ended:", result)

class EnemyBotCycleAttack(BotAI):
    NAME: str = "CycleAttackRetreat"
    RACE: Race = Race.Terran
    ...
    def __init__(self, args):
        ...
        self.phase = "attack"          # attack / retreat
    ...
    async def on_start(self):
        ...
        for item in coords[:3]:
            xy = tuple(map(int, item.split(",")))
            self.targets.append(Point2(xy))

        self.phase = "attack"
        self.current_target_index = 0
        self.phase_start_time = self.time

        print("EnemyBotCycleAttack started")
    ...
    async def on_step(self, iteration):
        ...
        if self.phase == "attack":
            await self.command_army_attack(target)

            if self.time - self.phase_start_time >= 10:
                self.phase = "retreat"
                self.phase_start_time = self.time

        # -------------------------
        # RETREAT PHASE
        # Pull back 20 map points
        # then rotate to next target
        # -------------------------
        elif self.phase == "retreat":
            fallback = self.get_retreat_point(target)

            await self.command_army_move(fallback)

            # stay retreating for 3 sec
            if self.time - self.phase_start_time >= 3:
                self.current_target_index = (self.current_target_index + 1) % 3
                self.phase = "attack"
                self.phase_start_time = self.time

    # ---------------------------------
    # All army attack target
    # ---------------------------------
    async def command_army_attack(self, target):
        army = self.get_army_units()

        for unit in army:
            unit.attack(target)

    # ---------------------------------
    # All army move fallback point
    # ---------------------------------
    async def command_army_move(self, target):
        army = self.get_army_units()
    ...
    # ---------------------------------
    # Get combat units only
    # ---------------------------------
    def get_army_units(self):
    ...
    # ---------------------------------
    # Pull back 20 points from target
    # toward own start location
    # ---------------------------------
    def get_retreat_point(self, target):
        home = self.start_location
        return target.towards(home, 40)

    async def on_end(self, result):
        print("EnemyBotCycleAttack ended:", result)
